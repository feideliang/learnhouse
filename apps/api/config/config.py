...
class TinybirdConfig(BaseModel):
    """ClickHouse-compatible analytics config.

    Works with self-hosted ClickHouse or Tinybird (managed ClickHouse).
    For ClickHouse: set api_url to http://host:8123, username, password.
    For Tinybird: set api_url to https://api.{region}.gcp.tinybird.co, leave username/password empty.
    """
    api_url: str        # ClickHouse: http://host:8123  |  Tinybird: https://api.{region}.gcp.tinybird.co
    username: str | None  # ClickHouse DB username (default: "default")
    password: str | None  # ClickHouse DB password
    # Tinybird legacy tokens — if set without username/password, falls back to Bearer auth
...
def get_learnhouse_config() -> LearnHouseConfig:
    ...
    env_access_key = os.environ.get("LEARNHOUSE_S3_API_ACCESS_KEY")
    env_secret_key = os.environ.get("LEARNHOUSE_S3_API_SECRET_KEY")
    bucket_name = (
    ...
    content_delivery = ContentDeliveryConfig(
        ...
        s3api=S3ApiConfig(bucket_name=bucket_name, endpoint_url=endpoint_url, access_key=access_key, secret_key=secret_key),  # type: ignore
    )
    ...
    env_ai_api_base_url = os.environ.get("LEARNHOUSE_AI_API_BASE_URL")
    env_ai_api_key = os.environ.get("LEARNHOUSE_AI_API_KEY")
    env_ai_model_name = os.environ.get("LEARNHOUSE_AI_MODEL_NAME")

    gemini_api_key = env_gemini_api_key or yaml_config.get("ai_config", {}).get(
        "gemini_api_key"
    )
    ai_api_base_url = env_ai_api_base_url or yaml_config.get("ai_config", {}).get(
        "ai_api_base_url", ""
    ) or None
    ai_api_key = env_ai_api_key or yaml_config.get("ai_config", {}).get(
        "ai_api_key", ""
    ) or None
    ai_model_name = env_ai_model_name or yaml_config.get("ai_config", {}).get(
        "ai_model_name", ""
    ) or None
    env_ai_embedding_model = os.environ.get("LEARNHOUSE_AI_EMBEDDING_MODEL")
    ai_embedding_model = env_ai_embedding_model or yaml_config.get("ai_config", {}).get(
        "ai_embedding_model", ""
    ) or None
    ...
    # Analytics config (ClickHouse or Tinybird) — auto-enabled when API URL is set
    env_analytics_api_url = os.environ.get("LEARNHOUSE_ANALYTICS_API_URL") or os.environ.get("LEARNHOUSE_TINYBIRD_API_URL")
    env_analytics_username = os.environ.get("LEARNHOUSE_ANALYTICS_USERNAME")
    env_analytics_password = os.environ.get("LEARNHOUSE_ANALYTICS_PASSWORD")
    env_analytics_ingest_token = os.environ.get("LEARNHOUSE_ANALYTICS_INGEST_TOKEN") or os.environ.get("LEARNHOUSE_TINYBIRD_INGEST_TOKEN")
    env_analytics_read_token = os.environ.get("LEARNHOUSE_ANALYTICS_READ_TOKEN") or os.environ.get("LEARNHOUSE_TINYBIRD_READ_TOKEN")

    analytics_api_url = env_analytics_api_url or yaml_config.get("tinybird_config", {}).get("api_url", "")
    analytics_username = env_analytics_username or yaml_config.get("tinybird_config", {}).get("username", "default")
    analytics_password = env_analytics_password or yaml_config.get("tinybird_config", {}).get("password", "")
    analytics_ingest_token = env_analytics_ingest_token or yaml_config.get("tinybird_config", {}).get("ingest_token", "")
    analytics_read_token = env_analytics_read_token or yaml_config.get("tinybird_config", {}).get("read_token", "")

    analytics_config = None
    if analytics_api_url:
        analytics_config = TinybirdConfig(
            api_url=analytics_api_url.rstrip("/"),
            username=analytics_username if analytics_username != "default" else "default",
            password=analytics_password,
            ingest_token=analytics_ingest_token,
            read_token=analytics_read_token,
        )
    ...
    config = LearnHouseConfig(
        ...
        tinybird_config=analytics_config,

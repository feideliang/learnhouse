...
def extract_jwt_from_request(request: Request) -> Optional[str]:
    """Extract JWT token from Authorization header, cookies, or ?token= query param.

    Precedence: Authorization header > cookie > query param. The query-param
    fallback exists so browser `<video>`/`<audio>`/`<iframe>` elements — which
    cannot set an Authorization header — can still authenticate to streaming
    and content-file endpoints when cookie auth is unavailable (e.g. direct
    cross-origin access without the Next.js proxy).
    """
    ...
    # Final fallback: ?token= query param for media tags that cannot set headers
...
async def get_current_user(
    ...
    api_token_value: Optional[str] = None
    if auth_header.lower().startswith("bearer lh_"):
        api_token_value = auth_header[7:].strip()
    else:
        # Browser media tags can't send Authorization headers — allow API tokens
        # via ?token=lh_... query param as well.
        query_token = request.query_params.get("token", "").strip()
        if query_token.startswith("lh_"):
            api_token_value = query_token

    if api_token_value:
        api_token_user = await validate_api_token(api_token_value, db_session)
        if api_token_user:

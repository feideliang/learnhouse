...
Uses OpenAI-compatible API for embeddings and
...
from src.services.ai.base import get_ai_client, LH_CONFIG
...
def get_embedding_model_name() -> str:
    ...
    return getattr(LH_CONFIG.ai_config, 'ai_embedding_model', None) or "text-embedding-3-small"
...
async def generate_embeddings(texts: list[str]) -> list[list[float]]:
    ...
    Runs blocking API calls off the event loop via asyncio.to_thread
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                for data in result.data:
                    all_embeddings.append(list(data.embedding))
                break

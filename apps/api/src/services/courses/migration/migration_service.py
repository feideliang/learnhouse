...
from src.db.users import PublicUser, APITokenUser, AnonymousUser
...
async def create_course_from_migration(
    ...
    # Reject anonymous users — creating a course requires a real user id
    # for the ResourceAuthor FK. Without this guard, AnonymousUser.id == 0
    # triggers a FK violation on resourceauthor_user_id_fkey.
    ...
    if not os.path.exists(manifest_real):
        raise HTTPException(
            status_code=404,
            detail=f"Migration package not found: {temp_id}",
        )
        ...
        for chapter_order, chapter_node in enumerate(structure.chapters):
            ...
            for act_order, act_node in enumerate(chapter_node.activities):
                ...
                for file_id in act_node.file_ids:
                    ...
                    if activity_type == ActivityTypeEnum.TYPE_VIDEO:
                        ...
                        if is_s3_enabled():
                            s3_key = f"orgs/{org_uuid}/courses/{course_uuid}/activities/{activity_uuid}/video/{saved_filename}"
                            ok = await asyncio.to_thread(upload_file_to_s3, s3_key, src_real)
                            if not ok:
                                raise RuntimeError(f"S3 upload failed for {s3_key}")
                        else:
                            files_dir = os.path.join(activity_dir, "video")
                            os.makedirs(files_dir, exist_ok=True)
                            dst_real = os.path.realpath(
                                os.path.join(files_dir, saved_filename)
                            )
                    ...
                    elif activity_type == ActivityTypeEnum.TYPE_DOCUMENT:
                        saved_filename = f"{new_file_id}.{ext}"
                        content = {
                            "filename": saved_filename,
                            "activity_uuid": activity_uuid,
                        ...
                        if is_s3_enabled():
                            s3_key = f"orgs/{org_uuid}/courses/{course_uuid}/activities/{activity_uuid}/documentpdf/{saved_filename}"
                            ok = await asyncio.to_thread(upload_file_to_s3, s3_key, src_real)
                            if not ok:
                                raise RuntimeError(f"S3 upload failed for {s3_key}")
                        else:
                            files_dir = os.path.join(activity_dir, "documentpdf")
                            os.makedirs(files_dir, exist_ok=True)
                            dst_real = os.path.realpath(
                                os.path.join(files_dir, saved_filename)
                            )

...
def record_failed_login(user: User, db_session: Session) -> Tuple[bool, Optional[int]]:
    ...
    """
    from sqlalchemy import update, case, func, cast, String
    ...
    # UPDATE required.
    # locked_until is stored as varchar (ISO-8601 string), so cast the
    # timestamp branch to String; otherwise Postgres rejects the CASE with
    # "types character varying and timestamp with time zone cannot be matched".
    stmt = (
        update(User)

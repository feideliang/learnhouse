...
/** Build WebSocket URL for collab server.
 * If a custom collab URL is configured (NEXT_PUBLIC_COLLAB_URL), use it.
 * Otherwise connect through the current page's host via Nginx /collab proxy.
 */
function getBoardCollabUrl(): string {
  const customUrl = getCollabUrl()
  if (customUrl !== 'ws://localhost:4000' && customUrl !== 'wss://localhost:4000') {
    return customUrl
  }
  // Use current page host via Nginx /collab proxy

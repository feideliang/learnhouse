...
// Maps backend BlockTypeEnum values to the corresponding TipTap node name.
const BLOCK_TYPE_TO_NODE: Record<string, string> = {
  BLOCK_AUDIO: 'blockAudio',
  BLOCK_IMAGE: 'blockImage',
  BLOCK_VIDEO: 'blockVideo',
  BLOCK_DOCUMENT_PDF: 'blockPDF',
}

// Node names whose blockObject consumers require a source_type='upload' marker
// (e.g. AudioBlockComponent gates playback on blockObject.source_type==='upload').
const UPLOAD_SOURCE_NODES = new Set(['blockAudio', 'blockVideo'])

/**
 * Converts the backend's simplified `{ blocks: [...] }` activity content into
 * a TipTap/ProseMirror doc. Files uploaded through the activity-creation
 * shortcut are stored as a flat blocks array and wouldn't render inside the
 * editor otherwise (useEditor silently drops unknown shapes).
 */
function normalizeBlocksToProseMirror(content: any, activityUuid?: string): any {
  if (
    !content ||
  ...
  const nodes = content.blocks
    .map((block: any) => {
      const nodeName = BLOCK_TYPE_TO_NODE[block?.block_type]
      if (!nodeName) return null
      const blockObject: any = {
        block_uuid: block.block_uuid,
        content: {
          ...(block.content || {}),
          ...(activityUuid && !block.content?.activity_uuid
            ? { activity_uuid: activityUuid }
            : {}),
        },
      }
      if (UPLOAD_SOURCE_NODES.has(nodeName)) blockObject.source_type = 'upload'
      return { type: nodeName, attrs: { blockObject } }
    })
    .filter(Boolean)
  ...
  const normalizedContent = useMemo(() => {
    ...
    try {
      const parsed = typeof props.content === 'string'
        ...
        : props.content;
      const shaped = normalizeBlocksToProseMirror(parsed, props.activity?.activity_uuid);
      return normalizeMarkTypes(shaped);
    } catch (e) {
      return props.content;
    }
  }, [props.content, props.activity?.activity_uuid]);

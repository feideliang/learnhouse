...
import { UserPen, ClockIcon, ArrowRight, BookOpen, UserPlus, LogOut, PlayCircle } from 'lucide-react'
...
function CoursesActions({ courseuuid, orgslug, course, trailData }: CourseActionsProps) {
  ...
  // Find the first activity that is not yet completed, falling back to the
  // first activity in the course. Used to resume playback when the user
  // re-enters a started course.
  const getResumeActivityUuid = (): string | null => {
    ...
    const firstIncomplete = allActivities.find(
      (a: any) => !completedIds.has(String(a.id)) && !completedIds.has(String(a.activity_uuid))
    );
    const target = firstIncomplete ?? allActivities[0];
    return target?.activity_uuid?.replace('activity_', '') ?? null;
  };

  const goToResumeActivity = () => {
    const activityUuid = getResumeActivityUuid();
    if (activityUuid) {
      router.push(
        getUriWithOrg(orgslug, '') + `/course/${courseuuid}/activity/${activityUuid}`
      );
    }
  };
  ...
  const handleCourseAction = async () => {
    ...
    // Already started — primary action is "continue": jump to the next
    // incomplete activity so the user can resume playback immediately.
    if (isStarted) {
      goToResumeActivity()
      return
    }
    ...
    const loadingToast = toast.loading(t('courses.start_course') + '...')
    ...
    try {
      ...
      toast.error(t('courses.start_course_error'), { id: loadingToast })
    } finally {
  ...
  const handleLeaveCourse = async () => {
    if (!session.data?.user || !isUserPartOfTheOrg) return
        ...
        <div className="bg-white nice-shadow rounded-lg overflow-hidden p-4">
          <div className="space-y-4">
            ...
            <button
              ...
              className="w-full py-2 rounded-lg font-medium text-sm transition-colors flex items-center justify-center gap-2 cursor-pointer text-red-600 hover:bg-red-50 disabled:text-red-400"
            >
              <LogOut className="w-4 h-4" />
              <span>{t('courses.leave_course')}</span>
            </button>
    ...
    <div className="bg-white shadow-md shadow-gray-300/25 outline outline-1 outline-neutral-200/40 rounded-lg overflow-hidden p-4">
      <div className="space-y-4">
        ...
        <button
          onClick={handleCourseAction}
          disabled={isActionLoading}
          aria-label={isStarted ? t('courses.continue_course') : t('courses.start_course')}
          className="w-full py-3 rounded-lg nice-shadow font-semibold transition-colors flex items-center justify-center gap-2 cursor-pointer bg-neutral-900 text-white hover:bg-neutral-800 disabled:bg-neutral-700"
        >
          {isActionLoading ? (
            <div className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" />
        ...
        {/* Secondary Leave button (only when started) */}
        {isStarted && (
          <button
            onClick={handleLeaveCourse}
            disabled={isActionLoading}

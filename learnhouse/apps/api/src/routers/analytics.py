...
# Lazy singleton httpx client for analytics Query API
...
def _get_read_client() -> httpx.AsyncClient | None:
    ...
    # ClickHouse: use Basic auth (username/password)
...
def _is_clickhouse() -> bool:
    """Detect if we're talking to ClickHouse (has username/password) vs Tinybird."""
    config = get_learnhouse_config()
    tb = config.tinybird_config
    if tb is None:
        return False
    return bool(tb.username and tb.password)
...
async def _execute_analytics_query(
    ...
    """
    Execute a SQL query via ClickHouse/Tinybird with Redis caching.

    1. Check Redis cache for a previous result.
    2. On miss, POST SQL to ClickHouse HTTP API or Tinybird /v0/sql.
...
# -------------------------------------------------------------------
# GET /status — Whether analytics is configured
# -------------------------------------------------------------------
@router.get(
    "/status",
    summary="Check analytics configuration",
    description="Returns whether the analytics backend (ClickHouse/Tinybird) is configured on the server.",
    responses={
...
async def query_dashboard_detail(
    ...
    ch_result = await _execute_analytics_query(
...
async def query_dashboard(
    ...
    result = await _execute_analytics_query(query_name, sql, safe_org_id, safe_days)
...
async def query_course_dashboard(
    ...
    result = await _execute_analytics_query(
        query_name, sql, safe_org_id, safe_days,
        course_id=safe_course_uuid,
    )
    result["data"] = _enrich_with_metadata(result.get("data", []), db_session)
...
async def export_analytics(
    ...
    for qname in query_names:
        ...
        result = await _execute_analytics_query(qname, sql, safe_org_id, d, course_id=safe_course_uuid)
        result["data"] = _enrich_with_metadata(result.get("data", []), db_session)

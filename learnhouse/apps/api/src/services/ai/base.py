...
def get_ai_client():
    """Get OpenAI-compatible client instance.

    Uses LEARNHOUSE_AI_API_BASE_URL + LEARNHOUSE_AI_API_KEY if configured,
    otherwise falls back to Gemini via LEARNHOUSE_GEMINI_API_KEY.
    """
    base_url = getattr(LH_CONFIG.ai_config, 'ai_api_base_url', None)
    api_key = getattr(LH_CONFIG.ai_config, 'ai_api_key', None)

    if base_url and api_key:
        return OpenAI(base_url=base_url, api_key=api_key)

    # Fallback: Gemini via OpenAI-compatible endpoint
    gemini_key = getattr(LH_CONFIG.ai_config, 'gemini_api_key', None)
    if gemini_key:
        return OpenAI(
    ...
    raise Exception("AI API key not configured. Set LEARNHOUSE_AI_API_BASE_URL and LEARNHOUSE_AI_API_KEY, or LEARNHOUSE_GEMINI_API_KEY.")


def _convert_contents_to_openai_format(contents: list[dict]) -> list[dict]:
...
def ask_ai(
    ...
    """
    Process an AI query using OpenAI-compatible SDK with course content as context
    """
    try:
        # Resolve model name from config if OpenAI-style model
        if not gemini_model_name or gemini_model_name.startswith("gemini-"):
            gemini_model_name = getattr(LH_CONFIG.ai_config, 'ai_model_name', None) or "gpt-4o-mini"

        client = get_ai_client()

        # Build conversation contents in Gemini-style format, then convert
        contents = []
        ...
        # Convert to OpenAI format and make the API call
        messages = _convert_contents_to_openai_format(contents)
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
            ...
            try:
                ...
                raise TimeoutError("AI API request timed out after 60s")
...
def generate_chat_title(user_message: str, ai_response: str) -> str:
    ...
    try:
        client = get_ai_client()
        model = getattr(LH_CONFIG.ai_config, 'ai_model_name', None) or "gpt-4o-mini"
        prompt = (
...
async def ask_ai_stream(
    ...
    """
    Process an AI query using OpenAI-compatible SDK with streaming response.
    ...
    try:
        ...
        system_instruction = f"{message_for_the_prompt}\n\nCourse Content Context:\n{text_reference}"
        contents.append({"role": "user", "parts": [{"text": system_instruction}]})
        contents.append({"role": "model", "parts": [{"text": "I understand. I'm ready to help with questions about this content."}]})
...
Questions:"""
        ...
        if response.choices and response.choices[0].message.content:
            ...
            questions = [q.strip().lstrip('0123456789.-) ') for q in content.strip().split('\n') if q.strip() and '?' in q]
            return questions[:3]

...
def build_attachment_parts_openai(attachments: List[AttachmentData]) -> list:
    """Build OpenAI-compatible content parts from attachments.

    - Images via data URL in image_url part
    - YouTube videos and documents via text description (not directly supported)
    """
    parts = []

    for attachment in attachments:
        if attachment.type == 'image' and attachment.content_base64 and attachment.mime_type:
            parts.append({
                "type": "image_url",
                "image_url": {
        ...
        elif attachment.type == 'youtube' and attachment.url:
            parts.append({
                "type": "text",
                "text": f"[YouTube Video: {attachment.url}]"
            })
        elif attachment.type == 'file' and attachment.content_base64 and attachment.mime_type:
            parts.append({
                "type": "text",
                "text": f"[Attached file: {attachment.name or 'document'} ({attachment.mime_type})]"
            })
...
async def generate_course_plan_stream(
    ...
    try:
        ...
        messages.append({"role": "assistant", "content": f"I understand. I'll create structured course plans in JSON format in {language_name}. I'll output only valid JSON with no additional text or formatting."})
        ...
        attachment_parts = build_attachment_parts_openai(attachments) if attachments else []
...
Please modify the plan according to the user's request. If any YouTube videos or documents were provided above, make sure to incorporate them into the relevant activities. Output ONLY the complete updated JSON plan."""
            user_content = [{"type": "text", "text": iteration_prompt}] + attachment_parts
...
IMPORTANT: You MUST incorporate the materials provided above into the course plan. For YouTube videos, include them in relevant activities using the blockEmbed block type with the exact URL provided."""
            ...
            user_content = [{"type": "text", "text": user_prompt}] + attachment_parts
            messages.append({"role": "user", "content": user_content})
...
async def generate_activity_content_stream(
    ...
    try:
        ...
        language_name = get_language_name(session.language)
        messages.append({"role": "system", "content": system_prompt})
        messages.append({"role": "assistant", "content": f"I understand. I'll generate educational content in ProseMirror JSON format in {language_name}. I'll output only valid JSON with no additional text."})

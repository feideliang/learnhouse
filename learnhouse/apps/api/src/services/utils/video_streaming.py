...
# Maximum file size that can be streamed without a Range header.
# Files larger than this will ALWAYS return 206 with a limited initial
# range (first 10MB), preventing full-file bandwidth saturation.
MAX_FILE_SIZE_FOR_FULL_STREAM = 50 * 1024 * 1024  # 50MB

# Maximum seconds a streaming connection can be idle before being closed.
STREAM_TIMEOUT_SECONDS = 120.0  # 2 minutes
...
def validate_video_path(base_content_dir: str, *path_parts: str) -> Optional[str]:
    ...
    if is_s3_enabled():
        # For S3, return the relative key path WITHOUT the base_content_dir prefix.
        # Migration uploads store files at "orgs/..." not "content/orgs/..." in S3.

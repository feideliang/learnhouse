...
const OrgEditSSO: React.FC = () => {
  ...
  // SourceID-specific
  const [sourceIdBaseUrl, setSourceIdBaseUrl] = useState('')
  const [sourceIdClientId, setSourceIdClientId] = useState('')
  const [sourceIdClientSecret, setSourceIdClientSecret] = useState('')
  const [sourceIdEmailDomain, setSourceIdEmailDomain] = useState('')
  ...
  const loadSSOData = async () => {
    ...
    try {
      ...
      if (configData) {
        ...
        // SourceID-specific
        setSourceIdBaseUrl(configData.provider_config?.base_url || '')
        setSourceIdClientId(configData.provider_config?.client_id || '')
        setSourceIdClientSecret(configData.provider_config?.client_secret || '')
        setSourceIdEmailDomain(configData.provider_config?.email_domain || '')
      }
    } catch (error) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      } else if (selectedProvider === 'sourceid') {
        providerConfig.base_url = sourceIdBaseUrl
        providerConfig.client_id = sourceIdClientId
        providerConfig.client_secret = sourceIdClientSecret
        if (sourceIdEmailDomain) {
          providerConfig.email_domain = sourceIdEmailDomain
        }
      }
          ...
          <div className="flex flex-col space-y-6 px-5 pb-4">
            ...
            {selectedProvider === 'sourceid' && (
              <div className="space-y-4 p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center gap-2 text-sm text-gray-600">
                  <Info className="w-4 h-4" />
                  锐捷 SourceID OAuth 2.0 单点登录（应用名：learnhouse）
                </div>

                <div className="space-y-2">
                  <Label htmlFor="sourceIdBaseUrl">
                    SourceID 服务器地址 (Base URL)
                  </Label>
                  <Input
                    id="sourceIdBaseUrl"
                    value={sourceIdBaseUrl}
                    onChange={(e) => setSourceIdBaseUrl(e.target.value)}
                    placeholder="https://sid.rghall.com.cn"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="sourceIdClientId">
                    应用 Client ID
                  </Label>
                  <Input
                    id="sourceIdClientId"
                    value={sourceIdClientId}
                    onChange={(e) => setSourceIdClientId(e.target.value)}
                    placeholder="your-client-id"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="sourceIdClientSecret">
                    应用 Client Secret
                  </Label>
                  <Input
                    id="sourceIdClientSecret"
                    type="password"
                    value={sourceIdClientSecret}
                    onChange={(e) => setSourceIdClientSecret(e.target.value)}
                    placeholder="your-client-secret"
                  />
                </div>

                <div className="space-y-2">
                  <Label htmlFor="sourceIdEmailDomain">
                    邮箱域名后缀（SourceID 无 email 字段，需手动构造）
                  </Label>
                  <Input
                    id="sourceIdEmailDomain"
                    value={sourceIdEmailDomain}
                    onChange={(e) => setSourceIdEmailDomain(e.target.value)}
                    placeholder="ruishan.cc"
                  />
                  <p className="text-xs text-gray-500">
                    例如：用户ID 为 zhangsan，域名后缀为 ruishan.cc，则邮箱为 zhangsan@ruishan.cc
                  </p>
                </div>
              </div>
            )}

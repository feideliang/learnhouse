...
import { LogIn, LogOut, ShoppingCart, Lock, UserPlus, PlayCircle } from 'lucide-react'
...
const CourseActionsMobile = ({ courseuuid, orgslug, course, trailData }: CourseActionsMobileProps) => {
  ...
  const handleCourseAction = async () => {
    ...
    // Already started — navigate to the first incomplete activity so the
    // user resumes playback instead of leaving the course.
    if (isStarted) {
      ...
      if (target) {
        router.push(
          ...
          `/course/${courseuuid}/activity/${target.activity_uuid.replace('activity_', '')}`
        )
      }
      return
    }
              ...
              {!!isStarted ? (
                <>
                  ...
                  <button
                    ...
                    disabled={isActionLoading}
                    className="w-full py-2 px-4 rounded-lg font-medium text-xs text-red-600 hover:bg-red-50 transition-colors flex items-center justify-center gap-2 disabled:text-red-400"
                  >

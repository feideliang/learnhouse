...
function InlineAudioPlayer({ src, title }: { src: string; title?: string }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <audio ref={audioRef} src={src} crossOrigin="use-credentials" preload="metadata" />
      ...
      {/* Error state */}
      {hasError && (
        <div className="px-4 pb-3 flex items-center gap-2 text-xs text-red-600 bg-red-50 rounded-b-xl">
          <AlertCircle size={14} />
          Audio failed to load. Please try refreshing the page.
        </div>
      )}
    </div>
  )
...
function PlaylistPlayer({
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
      <audio ref={audioRef} crossOrigin="use-credentials" preload="metadata" />
...
function AudioBlockComponent(props: ExtendedNodeViewProps) {
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      ...
      setError(null)
      setUploadProgress(0)
      const object = await uploadNewAudioFile(
        file,
  ...
  // Build audio URLs
  // Require all required params to be non-empty — avoid malformed URLs like /stream/block/audio/...//...
  const hasAllUploadParams =
    blockObject?.source_type === 'upload' &&
  ...
  const uploadAudioUrl = hasAllUploadParams

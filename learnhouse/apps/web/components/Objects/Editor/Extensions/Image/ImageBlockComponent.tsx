...
function ImageBlockComponent(props: any) {
  ...
  const handleUpload = async (file: File) => {
    if (!access_token) {
      const msg = 'Authentication required. Please log in and try again.'
      setError(msg)
      toast.error(msg)
      return
    }
  ...
  const handleDownload = () => {
    if (!hasAllImageParams) {
      toast.error('Cannot download: missing image or context information.')
      return
    }
  ...
  // Require all params to be non-empty to avoid malformed URLs
  const hasAllImageParams =
  ...
  const imageUrl = hasAllImageParams

...
const getLEARNHOUSE_BACKEND_URL = () => {
  const url = getConfig('NEXT_PUBLIC_LEARNHOUSE_BACKEND_URL', 'http://localhost/')
  return url.replace(/\/+$/, '') + '/'
}
...
export const getBackendUrl = () => {
  // Server-side: use non-prefixed env var (points to internal backend)
  const serverBackend = getConfig('LEARNHOUSE_BACKEND_URL')
